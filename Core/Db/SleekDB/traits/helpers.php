<?php
  ...
  trait HelpersTrait
  {
      private function init($conf = false)
      {
          ...
          $autoCache = true;
          if (isset($conf[ 'auto_cache' ])) {
              $autoCache = $conf[ 'auto_cache' ];
          }
      ...
      private function bootStore()
      {
          ...
          if (!$store || empty($store)) {
              throw new \Exception('Invalid store name was found');
          }
          // Prepare store name.
          if (substr($store, -1) !== '/') {
              $store = $store . '/';
          ...
          if (!file_exists($this->storePath)) {
              ...
              if (!mkdir($this->storePath, 0777, true)) {
                  throw new \Exception('Unable to create the store path at ' . $this->storePath);
              }
              // Create the cache directory.
              if (!mkdir($this->storePath . 'cache', 0777, true)) {
                  throw new \Exception('Unable to create the store\'s cache directory at ' . $this->storePath . 'cache');
              }
              // Create the data directory.
              if (!mkdir($this->storePath . 'data', 0777, true)) {
                  throw new \Exception('Unable to create the store\'s data directory at ' . $this->storePath . 'data');
              }
              // Create the store counter file.
              if (!file_put_contents($this->storePath . '_cnt.sdb', '0')) {
                  throw new \Exception('Unable to create the system counter for the store! Please check write permission');
              }
          }
          // Check if PHP has write permission in that directory.
          if (!is_writable($this->storePath)) {
              throw new \Exception('Store path is not writable at "' . $this->storePath . '." Please change store path permission.');
          }
          // Finally check if the directory is readable by PHP.
          if (!is_readable($this->storePath)) {
              throw new \Exception('Store path is not readable at "' . $this->storePath . '." Please change store path permission.');
          }
      }
      ...
      private function findStoreDocuments()
      {
          ...
          if (count($found) > 0) {
              ...
              // Skip data
              if ($this->skip > 0) {
                  $found = array_slice($found, $this->skip);
              }
              // Limit data.
              if ($this->limit > 0) {
      ...
      private function writeInStore($storeData)
      {
          ...
          if (isset($storeData[ '_id' ])) {
              throw new \Exception('The _id index is reserved by SleekDB, please delete the _id key and try again');
          }
          $id = $this->getStoreId();
          ...
          if ($storableJSON === false) {
              throw new \Exception('Unable to encode the data array, 
