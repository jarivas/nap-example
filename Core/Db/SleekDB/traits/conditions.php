<?php
  ...
  trait ConditionsTrait
  {
    ...
    // Add conditions to filter data.
      ...
      {
          if (empty($fieldName)) {
              throw new \Exception('Field name in where condition can not be empty.');
          }
          if (empty($condition)) {
      ...
      {
          if (empty($fieldName)) {
              throw new \Exception('Field name for in clause can not be empty.');
          }
          $values = (array) $values;
          $this->in[] = [
        'fieldName' => $fieldName,
      ...
      {
          if (empty($fieldName)) {
              throw new \Exception('Field name for notIn clause can not be empty.');
          }
          $values = (array) $values;
          $this->notIn[] = [
        'fieldName' => $fieldName,
      ...
      {
          if (empty($fieldName)) {
              throw new \Exception('Field name in orWhere condition can not be empty.');
          }
          if (empty($condition)) {
              throw new \Exception('The comparison operator can not be empty.');
          }
      ...
      {
          if (empty($field)) {
              throw new \Exception('Cant perform search doe to no field name was provided');
          }
          if (! empty($keyword)) {
              $this->searchKeyword = [
        'field'   => (array) $field,
