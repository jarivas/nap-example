<?php
  ...
  class SleekDB
  {
      ...
      {
          if (!$storeName or empty($storeName)) {
              throw new \Exception('Store name was not valid');
          }
      ...
      public function insertMany($storeData = false)
      {
          // Handle invalid data
          if (! $storeData or empty($storeData)) {
              throw new \Exception('No data found to insert in the store');
          }
          // Make sure that the data is an array
          if (! is_array($storeData)) {
              throw new \Exception('Data must be an array in order to insert in the store');
          }
          // All results.
          $results = [];
